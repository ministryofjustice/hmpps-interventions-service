...
class AmendReferralServiceUnitTest {
  ...
  inner class UpdateDraftReferralDesiredOutcomes {
    ...
    fun `can update multiple selected desired outcome for a service category`() {
      ...
      val desiredOutcome2 = DesiredOutcome(UUID.randomUUID(), "title", serviceCategoryId = serviceCategoryId1, deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
      val desiredOutcome3 = DesiredOutcome(UUID.randomUUID(), "title", serviceCategoryId = serviceCategoryId1, deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
      val desiredOutcome4 = DesiredOutcome(UUID.randomUUID(), "title", serviceCategoryId = serviceCategoryId1, deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
      val serviceCategory =
  ...
  inner class UpdateExpectedReleaseDate {
    ...
    fun `update expected release date for the referral`() {
      ...
      assertThat(oldExpectedReleaseDate).isEqualTo(formatter.format(existingExpectedReleaseDate))
    ...
    @Test
    fun `do not update expected release date for the referral when the existing and update values are same`() {
      whenever(userMapper.fromToken(jwtAuthenticationToken)).thenReturn(authUser)
      ...
      verifyNoInteractions(referralLocationRepository)
      verifyNoInteractions(changelogRepository)
      verifyNoInteractions(referralEventPublisher)
    }
