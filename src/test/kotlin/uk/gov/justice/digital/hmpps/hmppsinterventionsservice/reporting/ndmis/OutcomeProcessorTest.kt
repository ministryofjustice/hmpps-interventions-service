...
internal class OutcomeProcessorTest {
  ...
  fun `referral with single selected desired outcome correctly returns a outcomeData object`() {
    val desiredOutcome = DesiredOutcome(UUID.randomUUID(), "Outcome 1", UUID.randomUUID(), deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
  ...
  fun `referral with multiple selected service category correctly returns a outcomeData object`() {
    val desiredOutcome1 = DesiredOutcome(UUID.randomUUID(), "Outcome 1", UUID.randomUUID(), deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
    val desiredOutcome2 = DesiredOutcome(UUID.randomUUID(), "Outcome 2", UUID.randomUUID(), deprecatedAt = null, desiredOutcomeFilterRules = mutableSetOf())
    val endOfServiceReport = endOfServiceReportFactory.create(
      outcomes = mutableSetOf(
