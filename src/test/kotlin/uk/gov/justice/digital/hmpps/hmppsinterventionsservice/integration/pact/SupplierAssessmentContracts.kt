...
class SupplierAssessmentContracts(private val setupAssistant: SetupAssistant) {
  ...
  fun createSentReferralWithNoSaa() {
    // create a sent referral with no supplier assessment appointment
  ...
  fun createSentReferralWithSaaNoFeedback() {
    // create a sent referral with supplier assessment appointment with no feedback
  ...
  fun createSentReferralWithSaaWithoutFeedback() {
    // create a sent referral with supplier assessment without any feedback
  ...
  fun createSentReferralWithSaaAndAttendanceRecorded() {
    // create a sent referral with supplier assessment having attendance recorded
    ...
    setupAssistant.addSupplierAssessmentAppointment(referral.supplierAssessment!!, referral = referral, appointmentDeliveryType = AppointmentDeliveryType.PHONE_CALL, attended = attended)
  }

  @State("There is an existing sent referral with ID cd8f46a2-78f2-457b-ab14-7d77adce73d1 and the supplier assessment attendance and session feedback have been recorded")
  fun createSentReferralWithSaaAndBehaviourRecorded() {
    // create a sent referral with supplier assessment having behaviour recorded
  ...
  fun createSentReferralWithSaaHavingBehaviourReportedToNofifyingPP() {
    // create a sent referral with supplier assessment having behaviour recorded with notifying PP
    ...
    setupAssistant.addSupplierAssessmentAppointment(referral.supplierAssessment!!, referral = referral, appointmentDeliveryType = AppointmentDeliveryType.PHONE_CALL, attended = attended, additionalAttendanceInformation = additionalAttendanceInformation, notifyPPOfAttendanceBehaviour = notifyPPOfAttendanceBehaviour)
  }
}
