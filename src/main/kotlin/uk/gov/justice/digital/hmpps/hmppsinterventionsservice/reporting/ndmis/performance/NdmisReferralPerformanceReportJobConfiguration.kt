...
class NdmisReferralPerformanceReportJobConfiguration(
  ...
  @Bean
  fun ndmisReferralPerformanceReportJobLauncher(ndmisReferralPerformanceReportJob: Job): ApplicationRunner {
    return onStartupJobLauncherFactory.makeBatchLauncher(ndmisReferralPerformanceReportJob)
  }
  ...
  fun ndmisReferralPerformanceReportJob(
    ...
    return jobBuilderFactory["ndmisReferralPerformanceReportJob"]
  ...
  fun ndmisWriteReferralToCsvStep(
    @Qualifier("ndmisReferralReader") ndmisReader: HibernateCursorItemReader<Referral>,
  ...
  fun pushReferralToS3Step(@Value("#{jobParameters['outputPath']}") outputPath: String): Step =
    stepBuilderFactory["pushReferralToS3Step"].tasklet(pushFileToS3(outputPath), transactionManager).build()
