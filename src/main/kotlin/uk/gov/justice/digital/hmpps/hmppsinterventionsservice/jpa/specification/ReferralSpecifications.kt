...
class ReferralSpecifications {
  companion object {
    ...
    fun <T> sent(): Specification<T> = Specification<T> { root, _, cb ->
      cb.isNotNull(root.get<OffsetDateTime>("sentAt"))
    }

    fun <T> concluded(): Specification<T> = Specification<T> { root, _, cb ->
      cb.isNotNull(root.get<OffsetDateTime>("concludedAt"))
    }

    fun <T> unassigned(): Specification<T> = Specification<T> { root, _, cb ->
      cb.isEmpty(root.get<List<ReferralAssignment>>("assignments"))
    }

    fun <T> searchByPoPName(searchText: String): Specification<T> = Specification<T> { root, _, cb ->
    ...
    fun <T> searchByReferenceNumber(referenceNumber: String): Specification<T> = Specification<T> { root, _, cb -> cb.equal(root.get<String>("referenceNumber"), referenceNumber.uppercase()) }
     ...
     */
    fun <T> cancelled(): Specification<T> = Specification<T> { root, _, cb ->
      cb.and(
    ...
    fun <T> withSPAccess(contracts: Set<DynamicFrameworkContract>): Specification<T> = Specification<T> { root, _, _ ->
    ...
    fun <T> createdBy(authUser: AuthUser): Specification<T> = Specification<T> { root, _, cb -> cb.equal(root.get<String>("createdBy"), authUser) }

    fun <T> matchingServiceUserReferrals(serviceUserCRNs: List<String>): Specification<T> = Specification<T> { root, _, _ -> root.get<String>("serviceUserCRN").`in`(serviceUserCRNs) }

    inline fun <reified T> currentlyAssignedTo(authUserId: String): Specification<T> = Specification<T> { root, _, cb ->
