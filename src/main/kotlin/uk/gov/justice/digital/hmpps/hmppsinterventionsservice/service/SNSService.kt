  ...
  override fun onApplicationEvent(event: AppointmentEvent) {
    when (event.type) {
      AppointmentEventType.ATTENDANCE_RECORDED -> {
        ...
        val eventType = "intervention.session-appointment.${when (event.actionPlanSession.appointment.attended) {
        ...
        val snsEvent = EventDTO(
          ...
          event.detailUrl,
          event.actionPlanSession.appointment.attendanceSubmittedAt!!,
