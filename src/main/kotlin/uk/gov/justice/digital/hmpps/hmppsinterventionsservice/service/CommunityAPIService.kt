    ...
    when (event.type) {
      SESSION_FEEDBACK_RECORDED -> {
        ...
        val request = AppointmentOutcomeRequest(
          ...
          event.actionPlanSession.appointment.attended!!.name,
          event.actionPlanSession.appointment.notifyPPOfAttendanceBehaviour ?: false
        )

        val communityApiSentReferralPath = UriComponentsBuilder.fromPath(communityAPIAppointmentOutcomeLocation)
          .buildAndExpand(event.actionPlanSession.actionPlan.referral.serviceUserCRN, event.actionPlanSession.appointment.deliusAppointmentId, integrationContext)
          .toString()
